...
const CSS = `
.rr-shell { display: flex; min-height: 100vh; background: #ffffff; font-family: 'Inter', sans-serif; }
...
.rr-stat-card { background: #fff; border: 1px solid #E2E8F0; border-radius: 14px; padding: 16px; display: flex; flex-direction: column; gap: 10px; box-shadow: 0 1px 3px rgba(0,0,0,.01); transition: all 0.2s; }
.rr-stat-card:hover { transform: translateY(-1px); box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
.rr-stat-top { display: flex; align-items: center; justify-content: space-between; width: 100%; }
.rr-stat-ic-box { width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.rr-stat-lbl { font-size: 11px; color: #64748B; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }
.rr-stat-val { font-size: 22px; font-weight: 850; color: #0F172A; line-height: 1.1; margin: 2px 0; }
.rr-stat-bot { display: flex; align-items: center; justify-content: space-between; width: 100%; border-top: 1px solid #F3F4F6; padding-top: 8px; margin-top: 2px; }
.rr-stat-chg { font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; }
.rr-stat-chg.up { color: #10B981; }
.rr-stat-chg.dn { color: #EF4444; }
.rr-stat-sub { font-size: 10px; color: #94A3B8; margin-left: 2px; }
...
.rr-btn:hover { border-color: #2a195c; color: #2a195c; }
.rr-btn-primary { background: #2a195c; color: #fff; border-color: #2a195c; }
.rr-btn-primary:hover { background: #1E1044; border-color: #1E1044; color: #fff; }
        ...
        <div className="rr-main">
          ...
          <div className="rr-page">
            ...
            <div className="rr-stats-grid">
              {[
                { lbl: 'Total Rides', val: stats.total, ic: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>, bg: '#F5F3FF', clr: '#8B5CF6', chg: '10.5', up: true, pts: "0,28 14,22 28,18 40,20 52,12 64,15 76,8 82,10" },
                { lbl: 'Upcoming Rides', val: stats.upcoming, ic: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, bg: '#DCFCE7', clr: '#16A34A', chg: '12.3', up: true, pts: "0,30 14,24 28,18 40,14 52,18 64,10 76,13 82,6" },
                { lbl: 'Completed Rides', val: stats.completed, ic: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>, bg: '#FFF7ED', clr: '#F59E0B', chg: '5.2', up: true, pts: "0,22 14,20 28,24 40,16 52,20 64,13 76,17 82,11" },
                { lbl: 'Cancelled Rides', val: stats.cancelled, ic: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>, bg: '#FEE2E2', clr: '#EF4444', chg: '2.1', up: false, pts: "0,8 14,13 28,10 40,18 52,14 64,22 76,17 82,20" }
              ].map(s => (
                <div className="rr-stat-card" key={s.lbl}>
                  <div className="rr-stat-top">
                    <div className="rr-stat-ic-box" style={{ background: s.bg, color: s.clr }}>
                      {s.ic}
                    </div>
                  ...
                  <div>
                    <div className="rr-stat-lbl">{s.lbl}</div>
                    <div className="rr-stat-val" style={{ color: s.clr }}>{s.val}</div>
                  </div>
                  <div className="rr-stat-bot">
                    <span className={`rr-stat-chg ${s.up ? 'up' : 'dn'}`} style={{ fontSize: '11px', fontWeight: 'bold' }}>
                      {s.up ? '↑' : '↓'} {s.chg}%
                    </span>
                    <span className="rr-stat-sub">vs last week</span>
                  </div>
                </div>
              ))}
            </div>
